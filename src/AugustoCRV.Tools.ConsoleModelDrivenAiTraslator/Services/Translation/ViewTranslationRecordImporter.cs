...
internal sealed class ViewTranslationRecordImporter
{
    public void Import(IReadOnlyList<TranslationRecord> records, IOrganizationService service, bool forceUpdate)
    {
        ...
        var requests = new Dictionary<(Guid ViewId, string AttributeName), SetLocLabelsRequest>();
        ...
        foreach (var record in records)
        {
            ...
            if (!metadata.TryGetValue("View Id", out var viewIdValue) ||
                !Guid.TryParse(viewIdValue, out var viewId) ||
